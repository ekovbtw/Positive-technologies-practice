...
NTLM хэш успешно был получен злоумышленником. С помощью этого хэша можно подобрать пароль (например брутфорсом). Также можно пройти авторизацию на соседний сервер сети (переслать этот хэш, как-бы в реальном времени). 
